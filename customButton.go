...
func NewCustomButton(text string, icon fyne.Resource, tapped func()) *customButton {
	b := &customButton{text: text, icon: icon, onTapped: tapped}
...
func (b *customButton) CreateRenderer() fyne.WidgetRenderer {
	...
	return &customButtonRenderer{button: b, icon: icon, label: label, background: b.background,
		tapBG:   tapBG,
		objects: []fyne.CanvasObject{b.background, tapBG, icon, label}}
}
...
func (b *customButtonRenderer) Layout(size fyne.Size) {
	...
	b.label.Move(fyne.NewPos(iconSize.Width, 0))
}

func (b *customButtonRenderer) MinSize() fyne.Size {
	split := strings.Split(b.button.text, "\n")
	if len(split) > 0 {
		return fyne.NewSize(598, float32(len(split))*20) // set the size you want here
	} else {
